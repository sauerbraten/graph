...
var (
	ErrStartNotInGraph = errors.New("start node does not exist in graph")
	ErrEndNotInGraph   = errors.New("end node does not exist in graph")
	ErrNoPath          = errors.New("there exists no path from start node to end node in graph")
)
...
func (g *Graph) ShortestPathWithHeuristic(startKey, endKey string, heuristic func(key, endKey string) int) (path []string, err error) {
	g.mu.RLock()
	defer g.mu.RUnlock()
	...
	if start == nil {
		return nil, ErrStartNotInGraph
	}
	if end == nil {
		return nil, ErrEndNotInGraph
	}
